...
class FastAttention(nn.Module):
    ...
    def save_attn_gradients_q(self, attn_gradients):
        self.attn_gradients_q = attn_gradients.detach().cpu()

    def save_attn_gradients_k(self, attn_gradients):
        self.attn_gradients_k = attn_gradients.detach().cpu()

    def get_attn_gradients_k(self):
        return self.attn_gradients_k

    def get_attn_gradients_q(self):
        return self.attn_gradients_q

    def set_attn_q(self, q_attm):
        self.attn_q = q_attm.detach().cpu()
    
    def set_attn_k(self, k_attm):
        self.attn_k = k_attm.detach().cpu()
    ...
    def get_attn_ma(self):
        return self.attnout 

    def set_attn_ma(self,q,k):
        device = q.device
        v = torch.eye(q.size()[2]).repeat(q.size()[0], q.size()[1],1, 1).to(device)
        D_inv = 1. / torch.einsum('...nd,...d->...n', q, k.sum(dim = -2))
        context = torch.einsum('...nd,...ne->...de', k, v)
        self.attnout = torch.einsum('...de,...nd,...n->...ne', context, q, D_inv)
    ...
    def forward(self, q, k, v):
        ...
        #q.register_hook(self.save_attn_gradients_q)
        #k.register_hook(self.save_attn_gradients_k)
        attn_fn = linear_attention if not self.causal else self.causal_linear_fn
        #self.set_attn_k(k)
        #self.set_attn_q(q)
        #self.set_attn_ma(q.detach().cpu(),k.detach().cpu())
